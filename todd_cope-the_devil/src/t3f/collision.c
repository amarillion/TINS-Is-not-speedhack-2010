...
static void add_collision_point(T3F_COLLISION_LIST * lp, int x, int y)
{
...
T3F_COLLISION_OBJECT * t3f_create_collision_object(int rx, int ry, int w, int h, int tw, int th, int flags)
{
	...
	/* map top points */
	add_collision_point(&cp->map.top, rx + w / 2, ry); // center collision point
	for(i = rx; i < rx + w; i += tw)
	{
		add_collision_point(&cp->map.top, i, ry);
	}
	add_collision_point(&cp->map.top, rx + w - 1, ry);
    
	/* map bottom points */
	add_collision_point(&cp->map.bottom, rx + w / 2, ry + h - 1); // center collision point
	for(i = rx; i < rx + w; i += tw)
	{
		add_collision_point(&cp->map.bottom, i, ry + h - 1);
	}
	add_collision_point(&cp->map.bottom, rx + w - 1, ry + h - 1);

	/* map left points */
	add_collision_point(&cp->map.left, rx, ry + h / 2); // center collision point
	for(i = ry; i < ry + h; i += th)
	{
		add_collision_point(&cp->map.left, rx, i);
	}
	add_collision_point(&cp->map.left, rx, ry + h - 1);

	/* map right points */
	add_collision_point(&cp->map.right, rx + w - 1, ry + h / 2); // center collision point
	for(i = ry; i < ry + h; i += th)
	{
		add_collision_point(&cp->map.right, rx + w - 1, i);
	}
	add_collision_point(&cp->map.right, rx + w - 1, ry + h - 1);
	
	cp->flags = flags;
	return cp;
}

void t3f_recreate_collision_object(T3F_COLLISION_OBJECT * cp, int rx, int ry, int w, int h, int tw, int th, int flags)
{
	int i;
...
T3F_COLLISION_TILEMAP * t3f_load_collision_tilemap_entry(ALLEGRO_FILE * fp)
{
	...
	if(stricmp(header, "T3F_CTILEMAP"))
	{
		return NULL;
	}
	switch(header[15])
...
T3F_COLLISION_TILEMAP * t3f_load_collision_tilemap(char * fn)
{
	...
	tmp = t3f_load_collision_tilemap_entry(fp);
	al_fclose(fp);
	return tmp;
}

int t3f_save_collision_tilemap_entry(T3F_COLLISION_TILEMAP * tmp, ALLEGRO_FILE * fp)
{
	char header[16] = {0};
...
int t3f_save_collision_tilemap(T3F_COLLISION_TILEMAP * tmp, char * fn)
{
	ALLEGRO_FILE * fp;
	...
	t3f_save_collision_tilemap_entry(tmp, fp);
	al_fclose(fp);
	return 1;
}
