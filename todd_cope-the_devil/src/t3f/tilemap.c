...
void t3f_destroy_tileset(T3F_TILESET * tsp)
{
	...
	for(i = 0; i < tsp->tile_sheets; i++)
	{
		al_destroy_bitmap(tsp->tile_sheet[i]);
	}
	free(tsp);
}
...
T3F_TILESET * t3f_load_tileset_entry(ALLEGRO_FILE * fp)
{
	...
	switch(header[15])
	{
		case 0:
		{
			tsp->tile_sheet[0] = al_create_bitmap(512, 512);
			if(!tsp->tile_sheet[0])
			{
				return NULL;
			}
			t3f_start_sprite_sheet(tsp->tile_sheet[0], T3F_SPRITE_SHEET_TYPE_TILES);
			/* read tile data */
			tsp->tiles = al_fread16le(fp);
			for(i = 0; i < tsp->tiles; i++)
			{
				tsp->tile[i] = t3f_create_tile();
				tsp->tile[i]->ap = t3f_load_animation_entry(fp);
				t3f_add_animation_to_sprite_sheet(tsp->tile[i]->ap);
...
int t3f_save_tileset_entry(T3F_TILESET * tsp, ALLEGRO_FILE * fp)
{
	...
	for(i = 0; i < tsp->tiles; i++)
	{
		t3f_save_animation_entry(tsp->tile[i]->ap, fp);
	...
	switch(header[15])
	{
		case 0:
		{
			...
			for(i = 0; i < tmp->layers; i++)
			{
				...
				tmp->layer[i]->tile_sheet = al_fread16le(fp);
...
int t3f_save_tilemap_entry(T3F_TILEMAP * tmp, ALLEGRO_FILE * fp)
{
	...
	for(i = 0; i < tmp->layers; i++)
	{
		...
		al_fwrite16le(fp, tmp->layer[i]->tile_sheet);
...
int t3f_save_tilemap(T3F_TILEMAP * tmp, char * fn)
{
	...
	t3f_save_tilemap_entry(tmp, fp);
	al_fclose(fp);
	return 1;
}

float t3f_get_speed(T3F_TILEMAP * tmp, int layer, T3F_TILEMAP_CAMERA * cp)
{
	return (t3f_3d_project_x(1.0, tmp->layer[layer]->z - cp->z) - t3f_3d_project_x(0.0, tmp->layer[layer]->z - cp->z));
}

void t3f_select_camera(T3F_TILEMAP_CAMERA * cp)
{
	t3f_3d_select_projection(&cp->projection);
}

//extern ALLEGRO_FONT * mapper_font;
//extern int mapper_current_layer;
//extern T3F_TILEMAP * mapper_tilemap;
...
void t3f_render_tilemap(T3F_TILEMAP * tmp, int layer, int tick, T3F_TILEMAP_CAMERA * cp)
{
	...
	int tx, px;
	int ty, py;
	float zsp = t3f_get_speed(tmp, layer, cp);
	float ziw = (float)tmp->tileset->width * tmp->layer[layer]->scale;
	float zih = (float)tmp->tileset->height * tmp->layer[layer]->scale;
	float ztp = zsp * ziw;
	float zhp = zsp * zih;
	float sw;
	float sh;
	float cx = (cp->x * tmp->layer[layer]->speed_x) - tmp->layer[layer]->x;
	float cy = (cp->y * tmp->layer[layer]->speed_y) - tmp->layer[layer]->y;
	ALLEGRO_STATE old_blender;
	int revert = 0;
	
	if(cp->flags & T3F_TILEMAP_CAMERA_FLAG_NO_TRANSFORM)
	{
		sw = (float)cp->vw;
		sh = (float)cp->vh;
	}
	...
	{
		sw = (float)cp->vw / ((float)cp->vw / t3f_virtual_display_width);
		sh = (float)cp->vh / ((float)cp->vh / t3f_virtual_display_height);
	}
	...
	fox = (cx * zsp) / ztp - ((t3f_3d_project_x(0.0, tmp->layer[layer]->z - cp->z)) / ztp);
	ostartx = fox;
	if(fox < 0.0)
	{
		ostartx--;
	...
	foy = (cy * zsp) / zhp - ((t3f_3d_project_y(0.0, tmp->layer[layer]->z - cp->z)) / zhp);
	ostarty = foy;
	if(foy < 0.0)
	{
		ostarty--;
...
//	al_font_textprintf(mapper_font, 0, layer * 16, "%f - %f - %f", cx, zsp, tw);
	/* render the tiles */
	ty = ostarty;
	py = starty;
	...
	while(ty < ostarty + (int)th + 3)
	{
		...
		{
			if(layer == 0 || tmp->layer[layer]->data[py][px] != 0)
			{
				t3f_draw_scaled_animation(tmp->tileset->tile[t3f_get_tile(tmp->tileset, tmp->layer[layer]->data[py][px], tick)]->ap, al_map_rgba_f(1.0, 1.0, 1.0, 1.0), tick, tmp->layer[layer]->x + (float)tx * ziw - cp->x * tmp->layer[layer]->speed_x, tmp->layer[layer]->y + (float)ty * zih - cp->y * tmp->layer[layer]->speed_y, tmp->layer[layer]->z - cp->z, tmp->layer[layer]->scale, 0);
			}
			tx++;
			px++;
