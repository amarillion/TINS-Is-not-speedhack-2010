...
T3F_ANIMATION * t3f_load_animation(const char * fn)
{
	...
	ap = t3f_load_animation_entry(fp);
	al_fclose(fp);
	return ap;
}
...
int t3f_save_animation(T3F_ANIMATION * ap, const char * fn)
{
	...
	t3f_save_animation_entry(ap, fp);
	al_fclose(fp);
	return 1;
}
...
{
	int i;
	ALLEGRO_BITMAP * bp, * oldbp;
	
	for(i = 0; i < ap->bitmaps; i++)
	{
		oldbp = ap->bitmap[i];
		bp = t3f_add_bitmap_to_sprite_sheet(ap->bitmap[i]);
		ap->bitmap[i] = bp;
		al_destroy_bitmap(oldbp);
	}
}
...
void t3f_draw_animation(T3F_ANIMATION * ap, ALLEGRO_COLOR color, int tick, float x, float y, float z, int flags)
{
	...
	if(fp)
	{
		t3f_3d_draw_scaled_bitmap(ap->bitmap[fp->bitmap], color, x + fp->x, y + fp->y, z + fp->z, fp->width, fp->height, flags);
	}
}
...
void t3f_draw_scaled_animation(T3F_ANIMATION * ap, ALLEGRO_COLOR color, int tick, float x, float y, float z, float scale, int flags)
{
	...
	if(fp)
	{
		t3f_3d_draw_scaled_bitmap(ap->bitmap[fp->bitmap], color, x + fp->x, y + fp->y, z + fp->z, fp->width * scale, fp->height * scale, flags);
	}
}
...
void t3f_draw_rotated_animation(T3F_ANIMATION * ap, ALLEGRO_COLOR color, int tick, float cx, float cy, float x, float y, float z, float angle, int flags)
{
	...
	if(fp)
	{
		...
		t3f_3d_draw_rotated_scaled_bitmap(ap->bitmap[fp->bitmap], color, cx / scale_x, cy / scale_y, x + fp->x, y + fp->y, z + fp->z, angle, scale_x, scale_y, flags);
	}
}
...
void t3f_draw_rotated_scaled_animation(T3F_ANIMATION * ap, ALLEGRO_COLOR color, int tick, float cx, float cy, float x, float y, float z, float angle, float scale, int flags)
{
	...
	if(fp)
	{
		t3f_3d_draw_rotated_scaled_bitmap(ap->bitmap[fp->bitmap], color, cx, cy, x + fp->x, y + fp->y, z + fp->z, angle, scale, scale, flags);
	}
}
