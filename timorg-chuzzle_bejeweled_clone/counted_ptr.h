...
public:
    ...
    // timmy added
    bool operator==(const counted_ptr<X> &c) const
    {
        return (get() == c.get());
    }
    bool operator!=(const counted_ptr<X> &c) const
...
// comparison operator to allow for std::list<counted_ptr<TILE> >::merge() to work
template <class T>
bool operator<(counted_ptr<T>, counted_ptr<T>)
{
    return false;
}
